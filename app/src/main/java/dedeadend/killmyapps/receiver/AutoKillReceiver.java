    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (Intent.ACTION_BOOT_COMPLETED.equals(action)) {
            if (App.settings.getBoolean(App.FIXED_TIME_AUTO_KILL, false)) {
                scheduleDailyAlarm(
                        context,
                        App.settings.getInt(App.FIXED_TIME_AUTO_KILL_HOUR, 3),
                        App.settings.getInt(App.FIXED_TIME_AUTO_KILL_MINUTE, 30)
                );
            }
            ...
            if (App.settings.getBoolean(App.SCREEN_OFF_AUTO_KILL, false)) {
                ...
                if (hasPermission) {
                    int delay = App.settings.getInt(App.SCREEN_OFF_AUTO_KILL_DELAY, 0);
                    AutoKillHelper.enableScreenOffKill(context, delay);
                }
            }
            return;
        }
